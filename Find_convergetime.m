...
for i=152:181
    ...
    for j=1:2880
        flag=0;%用于刷新整个寻找过程
        if(abs(Data_total(j,7,i-151))<0.10)
        ...
        for k=j:2880
           ...
           if(abs(Data_total(k,7,i-151))>0.10&&t<20)
               flag2=1;
               break
           end
           if(flag2==1)
    ...
    for j=1:2880
        flag=0;%用于刷新整个寻找过程
        if(abs(Data_total(j,8,i-151))<0.10)
            hour=Data_total(j,4,i-151);
            min=Data_total(j,5,i-151);
            sec=Data_total(j,6,i-151);
            converge_row_E=j;
        ...
        for k=j:2880
           flag2=0;%用于跳出检验阶段循环
           %如果收敛后又反弹且保持的历元不足20，则跳出整个循环，从下一行重新寻找
           if(abs(Data_total(k,8,i-151))>0.10&&t<20)
               flag2=1;
               break
           end
           if(flag2==1)
    ...
    for j=1:2880
        flag=0;%用于刷新整个寻找过程
        if(abs(Data_total(j,9,i-151))<0.10)
            hour=Data_total(j,4,i-151);
            min=Data_total(j,5,i-151);
            sec=Data_total(j,6,i-151);
            converge_row_U=j;
        ...
        for k=j:2880
           flag2=0;%用于跳出检验阶段循环
           %如果收敛后又反弹且保持的历元不足20，则跳出整个循环，从下一行重新寻找
           if(abs(Data_total(k,9,i-151))>0.10&&t<20)
               flag2=1;
               break
           end
           if(flag2==1)
    ...
    for j=1:2880
        flag=0;%用于刷新整个寻找过程
        if(abs(Data_total(j,10,i-151))<0.10)
            hour=Data_total(j,4,i-151);
            min=Data_total(j,5,i-151);
            sec=Data_total(j,6,i-151);
            converge_row_R=j;
        ...
        for k=j:2880
           flag2=0;%用于跳出检验阶段循环
           %如果收敛后又反弹且保持的历元不足20，则跳出整个循环，从下一行重新寻找
           if(abs(Data_total(k,10,i-151))>0.10&&t<20)
               flag2=1;
               break
           end
           if(flag2==1)
    ...
    for j=1:2880
        ...
        for k=j:2880
           flag2=0;%用于跳出检验阶段循环
           %如果收敛后又反弹且保持的历元不足20，则跳出整个循环，从下一行重新寻找
           if(Data_total(j,13,i-151)>0.10&&t<20)
               flag2=1;
               break
           end
           if(flag2==1)
    ...
    %N方向误差的rms
    Analy_total(i-151,5)=Data_total(maxrow,7,i-151);
    %E方向误差的rms
    Analy_total(i-151,6)=Data_total(maxrow,8,i-151);
    %H方向误差的rms
    Analy_total(i-151,12)=Data_total(maxrow,13,i-151);
    %U方向误差的rms
    Analy_total(i-151,7)=Data_total(maxrow,9,i-151);
    %R方向误差的rms
    Analy_total(i-151,8)=Data_total(maxrow,10,i-151);
    %计算平均PDOP
...
for i=152:181
    ...
    %输出rms
    fprintf(fid2,'%5.2f    %5.2f   %5.2f   %5.2f   %5.2f            ',abs(Analy_total(i-151,5)*100),abs(Analy_total(i-151,6)*100),abs(Analy_total(i-151,12)*100),abs(Analy_total(i-151,7)*100),abs(Analy_total(i-151,8)*100)); 
    %输出平均PDOP
